...
from ..utils.security import PasswordHasher, TokenManager, PasswordValidator
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                ...
                detail={"message": "비밀번호가 보안 요구사항을 충족하지 않습니다", "errors": errors}
            )
...
async def login(
    ...
    try:
        ...
        # Verify password
        if not password_hasher.verify_password(user_credentials.password, user.password_hash):
            # Log failed attempt
        ...
        if cache_service.redis_client:
            try:
                await cache_service.redis_client.delete(f"login:failed:{user_credentials.email}")
            except Exception:
                pass
...
async def logout(
    ...
    try:
        ...
        logger.info(f"User logged out: {getattr(request.state, 'user_email', 'unknown')}")
...
async def change_password(
    ...
    try:
        ...
        if not password_hasher.verify_password(password_data.current_password, user.password_hash):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        if not is_valid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"message": "새 비밀번호가 보안 요구사항을 충족하지 않습니다", "errors": errors}
            )
