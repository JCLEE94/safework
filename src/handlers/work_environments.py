...
from src.schemas.work_environment import (WorkEnvironmentCreate,
...
async def create_work_environment(
    ...
    """작업환경측정 기록 생성"""
    from src.models.work_environment import MeasurementResult, MeasurementType
...
async def get_environment_statistics(db: AsyncSession = Depends(get_db)):
    ...
    result_stats = await db.execute(
        select(
            WorkEnvironment.result, func.count(WorkEnvironment.id).label("count")
    ...
    re_measurement_count = await db.scalar(
        select(func.count(WorkEnvironment.id)).where(
            WorkEnvironment.re_measurement_required == "Y"
        )
    )
...
async def get_compliance_status(db: AsyncSession = Depends(get_db)):
    ...
    all_locations = await db.execute(select(WorkEnvironment.location).distinct())
    ...
    for location in locations:
        ...
        location_status = {"location": location, "measurements": []}
    ...
    return {"locations": compliance_status, "total_locations": len(locations)}
...
async def get_work_environment(env_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not work_env:
        raise HTTPException(
            status_code=404, detail="작업환경측정 기록을 찾을 수 없습니다"
        )
...
async def add_worker_exposures(
    ...
    for exposure_data in exposures:
        ...
        existing = await db.scalar(
            select(WorkEnvironmentWorkerExposure).where(
                and_(
        ...
        exposure = WorkEnvironmentWorkerExposure(
            work_environment_id=env_id, **exposure_data.model_dump()
        )
        db.add(exposure)
        added_count += 1
