...
class ReportGenerator:
    ...
    async def generate_worker_health_summary(self, filters: ReportFilters) -> bytes:
        ...
        for row, worker in enumerate(workers, 2):
            ...
            ws.cell(
                row=row,
                column=5,
                value=worker.hire_date.strftime("%Y-%m-%d") if worker.hire_date else "",
            )
            ws.cell(
                row=row,
                column=6,
                value=worker.health_status.value if worker.health_status else "",
            )

            if latest_exam:
                ws.cell(
                    row=row, column=7, value=latest_exam.exam_date.strftime("%Y-%m-%d")
                )
                # 차기 검진일
                next_exam = latest_exam.exam_date + timedelta(
        ...
        health_stats = Counter(
            [w.health_status.value if w.health_status else "미정" for w in workers]
        )

        stats_ws.cell(row=1, column=1, value="건강상태별 통계").font = Font(
            bold=True, size=14
        )
    ...
    async def generate_work_env_compliance(self, filters: ReportFilters) -> bytes:
        ...
        if filters.start_date:
            conditions.append(
                WorkEnvironment.measurement_date
        ...
        summary_data = [
            ...
            [
                "적합률",
                (
                    f"{(pass_count/total_measurements*100):.1f}%"
    ...
    async def generate_monthly_dashboard(self, filters: ReportFilters) -> bytes:
        ...
        work_env_query = select(func.count(WorkEnvironment.id)).where(
            and_(
                WorkEnvironment.measurement_date >= start_date,
                WorkEnvironment.measurement_date <= end_date,
            )
        )
        ...
        accident_query = select(func.count(AccidentReport.id)).where(
            and_(
                AccidentReport.accident_date >= start_date,
                AccidentReport.accident_date <= end_date,
            )
        )
        ...
        # 제목
        period = (
            f"{start_date.strftime('%Y년 %m월')} ~ {end_date.strftime('%Y년 %m월')}"
        )
...
async def generate_report(request: Dict[str, Any], db: AsyncSession = Depends(get_db)):
    """보고서 생성"""
    ...
    try:
        if template_id == "worker_health_summary":
            ...
            filename = (
                f"근로자_건강현황_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            )
            media_type = (
            ...
            filename = (
                f"작업환경측정_법규준수_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
            )
            ...
            content = f"보고서 템플릿 '{template_id}'는 아직 구현되지 않았습니다.\n생성 요청: {datetime.now()}".encode(
                "utf-8"
            )
            filename = (
                f"보고서_{template_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
