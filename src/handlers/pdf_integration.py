...
from src.testing import (assert_korean_text, assert_pdf_valid,
                         assert_response_ok, create_test_environment,
                         integration_test, measure_performance,
                         run_inline_tests)
...
class IntegrationTestPDFGeneration:
    ...
    async def test_health_certificate_generation(self):
        ...
        async with create_test_environment() as env:
            ...
            async with db.get_session() as session:
                from src.models import HealthExam, Worker
            ...
            response = await client.post(
                "/api/v1/documents/fill-pdf/health_certificate", json=pdf_data
            )
            ...
            pdf_info = assert_pdf_valid(
                result["pdf_base64"], min_pages=1, check_korean=True
            )
    ...
    async def test_work_environment_report_generation(self):
        ...
        async with create_test_environment() as env:
            ...
            response = await client.post(
                "/api/v1/documents/fill-pdf/environment_report", json=pdf_data
            )
    ...
    async def test_msds_summary_generation(self):
        ...
        async with create_test_environment() as env:
            ...
            response = await client.post(
                "/api/v1/documents/fill-pdf/msds_summary", json=pdf_data
            )
    ...
    async def test_bulk_pdf_generation(self):
        ...
        async with create_test_environment() as env:
            ...
            assert (
                total_success >= 45
            ), f"너무 많은 PDF 생성 실패: {50 - total_success}개"
            assert duration < 30.0, f"대량 PDF 생성이 너무 느립니다: {duration:.2f}초"

            avg_time = duration / 50
            print(
                f"✅ 50개 PDF 생성 완료: {duration:.2f}초 (평균 {avg_time:.2f}초/PDF)"
            )
    ...
    async def test_pdf_template_coordinates(self):
        ...
        async with create_test_environment() as env:
            ...
            for form_name in forms:
                # 최소 데이터로 PDF 생성 시도
                response = await client.post(
                    f"/api/v1/documents/fill-pdf/{form_name}", json=test_fields
                )
    ...
    async def test_pdf_error_handling(self):
        ...
        async with create_test_environment() as env:
            ...
            # 1. 잘못된 양식 이름
            response = await client.post(
                "/api/v1/documents/fill-pdf/invalid_form_name", json={"test": "data"}
            )
            assert response.status_code == 404

            # 2. 필수 데이터 누락
            response = await client.post(
                "/api/v1/documents/fill-pdf/health_certificate", json={}  # 빈 데이터
            )
    ...
    async def test_pdf_concurrent_generation(self):
        ...
        async with create_test_environment() as env:
            ...
            # 20개 동시 요청
            results = await asyncio.gather(*[generate_pdf(i) for i in range(20)])
            ...
            assert (
                success_rate >= 0.9
            ), f"동시 요청 성공률이 너무 낮습니다: {success_rate:.0%}"
