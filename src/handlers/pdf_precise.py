...
class PrecisePDFProcessor:
        ...
        try:
                ...
                for widget in page.widgets():
                    if widget.field_name:
                        ...
                        field_info = {
                            ...
                            "type": self._determine_field_type(
                                widget.field_type_string, widget.field_name
                            ),
                            "coordinates": {
                                "x": float(rect.x0),
                                "y": float(rect.y0),
            ...
            with pdfplumber.open(io.BytesIO(pdf_content)) as pdf:
                for page_num, page in enumerate(pdf.pages):
                    ...
                    # 빈 텍스트 박스나 밑줄 패턴 감지
                    detected_patterns = self._detect_text_patterns(
                        words, chars, page_num + 1
                    )
    ...
    def _determine_field_type(self, widget_type: str, field_name: str) -> str:
        ...
        if "text" in widget_type.lower():
            ...
            if any(pattern in field_name_lower for pattern in ["date", "날짜", "일자"]):
                ...
                pattern in field_name_lower for pattern in ["sign", "서명", "사인"]
            ):
                return "signature"
            else:
                return "text"
    ...
    def _detect_text_patterns(
        self, words: List[Dict], chars: List[Dict], page_num: int
    ) -> List[Dict[str, Any]]:
        """텍스트 패턴 기반 필드 감지"""
        detected_fields = []
        ...
        for i, word in enumerate(words):
            ...
            if (
                ...
                # 입력 영역 추정 (라벨 옆의 빈 공간)
                input_area = self._estimate_input_area(
                    word, words[i + 1 : i + 3] if i + 1 < len(words) else []
                )
    ...
    def _estimate_input_area(
        self, label_word: Dict, next_words: List[Dict]
    ) -> Optional[Dict]:
        """라벨 다음의 입력 영역 추정"""
        ...
        for next_word in next_words:
            if (
                next_word["x0"] < input_area["x1"]
                and next_word["top"] < input_area["bottom"]
                and next_word["bottom"] > input_area["top"]
            ):
                # 겹치면 너비 조정
    ...
    def _estimate_ocr_input_area(
        self, ocr_data: Dict, label_index: int, image_size: Tuple[int, int]
    ) -> Optional[Dict]:
        """OCR 데이터에서 입력 영역 추정"""
    ...
    def _filter_and_deduplicate_fields(
    ...
    def _draw_signature(self, page, coords: Dict, name: str):
        ...
        # 서명선 그리기
        line_y = rect.y1 - 5
        page.draw_line((rect.x0, line_y), (rect.x1, line_y), color=(0, 0, 0), width=1)
...
async def detect_precise_fields(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {
            ...
            "methods_used": list(
                set(field.get("method", "unknown") for field in detected_fields)
            ),
        }
