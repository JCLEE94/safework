...
class ComplianceMonitoringService:
    ...
    async def check_single_compliance(
        ...
        result_dict["next_check"] = self.calculate_next_check(
            current_time, rule.check_frequency
        ).isoformat()

        await self.cache_service.set(
            cache_key, result_dict, expire=86400 * 7
        )  # 7일 캐시
    ...
    async def perform_compliance_check(
        ...
        if rule.id == "safety_manager_appointment_2024":
            ...
            return ComplianceCheckResult(
                ...
                next_check=self.calculate_next_check(
                    current_time, rule.check_frequency
                ),
            )

    async def check_safety_manager_appointment(
        self, rule: ComplianceRule
    ) -> ComplianceCheckResult:
        """보건관리자 선임 점검"""
        ...
        return ComplianceCheckResult(
            ...
            next_check=self.calculate_next_check(
                datetime.utcnow(), rule.check_frequency
            ),
    ...
    async def check_special_health_exam(
        self, rule: ComplianceRule
    ) -> ComplianceCheckResult:
        """특수건강진단 점검"""
    ...
    async def check_safety_education(
        self, rule: ComplianceRule
    ) -> ComplianceCheckResult:
        """안전교육 점검"""
        ...
        if len(incomplete_education) > 0:
            status = ComplianceStatus.VIOLATION
            action_required = (
                f"{len(incomplete_education)}명 기초안전보건교육 6시간 이수 필요"
            )
        else:
            status = ComplianceStatus.COMPLIANT
            action_required = None
    ...
    async def check_noise_measurement(
        self, rule: ComplianceRule
    ) -> ComplianceCheckResult:
        """소음측정 점검"""
        ...
        for area in high_noise_areas:
            ...
            if last_measurement_date:
                days_since_measurement = (
                    datetime.utcnow() - last_measurement_date
                ).days
        ...
        if len(overdue_measurements) > 0:
            status = ComplianceStatus.VIOLATION
            action_required = (
                f"{len(overdue_measurements)}개 구역 소음측정 즉시 실시 필요"
            )
        else:
            status = ComplianceStatus.COMPLIANT
            action_required = None
    ...
    async def check_msds_management(
        self, rule: ComplianceRule
    ) -> ComplianceCheckResult:
        """MSDS 관리 점검"""
    ...
    async def check_executive_inspection(
        self, rule: ComplianceRule
    ) -> ComplianceCheckResult:
        """경영책임자 점검 의무 확인"""
    ...
    def calculate_next_check(
        self, last_check: datetime, frequency: CheckFrequency
    ) -> datetime:
        """다음 점검일 계산"""
    ...
    async def create_compliance_alert(
        self, rule: ComplianceRule, result: ComplianceCheckResult
    ):
        """법령 준수 알림 생성"""
        ...
        return [
            {
                "id": 1,
                "name": "홍길동",
                "last_special_exam_date": datetime.utcnow() - timedelta(days=100),
            },
            {
                "id": 2,
                "name": "김철수",
                "last_special_exam_date": datetime.utcnow() - timedelta(days=130),
            },
        ]
        ...
        return [
            {
                "id": 1,
                "name": "용접구역",
                "last_noise_measurement_date": datetime.utcnow() - timedelta(days=95),
            },
            {
                "id": 2,
                "name": "절단구역",
                "last_noise_measurement_date": datetime.utcnow() - timedelta(days=80),
            },
        ]
        ...
        return [
            {
                "id": 1,
                "name": "톨루엔",
                "msds_last_updated": datetime.utcnow() - timedelta(days=700),
            },
...
async def check_all_compliance(
    ...
    try:
        ...
        warning_count = len(
            [r for r in results if r.status == ComplianceStatus.WARNING]
        )
        violation_count = len(
            [r for r in results if r.status == ComplianceStatus.VIOLATION]
        )
        ...
        return {
            ...
            "summary": {
                ...
                "compliance_rate": (
                    round((compliant_count / total_rules) * 100, 1)
                    if total_rules > 0
                    else 0
                ),
            },
        ...
        raise HTTPException(
            status_code=500, detail="법령 준수 점검 중 오류가 발생했습니다"
        )
...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        high_priority_violations = [
            ...
            if r.status == ComplianceStatus.VIOLATION
            and any(
                rule.penalty_level in [PenaltyLevel.HIGH, PenaltyLevel.CRITICAL]
                for rule in COMPLIANCE_RULES
                if rule.id == r.rule_id
            )
        ]
        ...
        raise HTTPException(
            status_code=500, detail="대시보드 데이터 조회 중 오류가 발생했습니다"
        )
