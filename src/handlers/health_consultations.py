...
from ..models.health_consultation import (ConsultationFollowUp,
                                          HealthConsultation)
from ..models.worker import Worker
from ..schemas.health_consultation import (ConsultationStatistics,
                                           ConsultationStatus,
                                           ConsultationType,
    ...
    try:
        ...
        for consultation in consultations:
            consultation_data = {
                ...
                "worker_name": (
                    consultation.worker.name if consultation.worker else None
                ),
                "worker_employee_id": (
                    consultation.worker.employee_id if consultation.worker else None
                ),
                "created_at": consultation.created_at,
                "updated_at": consultation.updated_at,
            }
            consultation_responses.append(
                HealthConsultationResponse(**consultation_data)
            )

        return HealthConsultationListResponse(
            items=consultation_responses, total=total, skip=skip, limit=limit
        )
...
@router.post(
    "/", response_model=HealthConsultationResponse, status_code=status.HTTP_201_CREATED
)
async def create_consultation(
    consultation_data: HealthConsultationCreate, db: AsyncSession = Depends(get_db)
):
    """보건상담 등록"""
    ...
    try:
        ...
        consultation_query = (
            ...
            .options(selectinload(HealthConsultation.worker))
        ...
        return HealthConsultationResponse.model_validate(
            consultation_with_worker, from_attributes=True
        )
...
async def get_consultation(consultation_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return HealthConsultationResponse.model_validate(
            consultation, from_attributes=True
        )
...
async def update_consultation(
    ...
    try:
        ...
        # 기존 상담 조회
        query = select(HealthConsultation).where(
...
async def delete_consultation(consultation_id: int, db: AsyncSession = Depends(get_db)):
    """보건상담 삭제"""
...
async def get_consultation_statistics(
    ...
    try:
        ...
        # 상담 유형별 통계
        type_query = select(
            HealthConsultation.consultation_type, func.count().label("count")
        ...
        monthly_query = (
            select(
                func.extract("year", HealthConsultation.consultation_date).label(
                    "year"
                ),
                func.extract("month", HealthConsultation.consultation_date).label(
                    "month"
                ),
