...
from src.models.health_exam_appointment import (AppointmentNotification,
                                                AppointmentStatus,
                                                HealthExamAppointment,
                                                NotificationType)
from src.models.worker import Worker
from src.schemas.health_exam_appointment import (
    AppointmentListResponse, AppointmentReminderRequest, AppointmentStatistics,
    BulkAppointmentCreate, HealthExamAppointmentCreate,
    HealthExamAppointmentResponse, HealthExamAppointmentStatusUpdate,
    HealthExamAppointmentUpdate)
from src.services.notifications import NotificationService
from src.utils.auth_deps import get_current_user_id
from src.utils.logger import logger

router = APIRouter(
    prefix="/api/v1/health-exam-appointments", tags=["health-exam-appointments"]
)
...
async def create_appointment(
    ...
    existing = await db.execute(
        select(HealthExamAppointment).where(
            and_(
                ...
                HealthExamAppointment.status.in_(
    ...
    appointment = HealthExamAppointment(
        **appointment_data.model_dump(exclude={"notification_methods", "exam_items"}),
        notification_methods=json.dumps(
            [m.value for m in appointment_data.notification_methods]
        ),
...
async def get_appointments(
    ...
    """건강진단 예약 목록 조회"""
    query = select(HealthExamAppointment).options(
        selectinload(HealthExamAppointment.worker)
    )
    ...
    if medical_institution:
        query = query.where(
            HealthExamAppointment.medical_institution.contains(medical_institution)
        )
    if exam_type:
...
async def get_appointment_statistics(
    ...
    status_stats = await db.execute(
        select(HealthExamAppointment.status, func.count(HealthExamAppointment.id))
        .where(
            and_(
    ...
    # 비율 계산
    total = sum(
        [stats["total_completed"], stats["total_cancelled"], stats["total_no_show"]]
    )
    stats["completion_rate"] = (
        (stats["total_completed"] / total * 100) if total > 0 else 0
    )
    stats["no_show_rate"] = (stats["total_no_show"] / total * 100) if total > 0 else 0

    # 검진 유형별 통계
    type_stats = await db.execute(
        select(HealthExamAppointment.exam_type, func.count(HealthExamAppointment.id))
        .where(
            and_(
                HealthExamAppointment.scheduled_date >= date_from,
                HealthExamAppointment.scheduled_date <= date_to,
    ...
    stats["by_institution"] = {
        institution: count for institution, count in institution_stats
    }
...
async def get_appointment(appointment_id: int, db: AsyncSession = Depends(get_db)):
    ...
    appointment = await db.get(
        HealthExamAppointment,
        appointment_id,
        options=[selectinload(HealthExamAppointment.worker)],
    )
    if not appointment:
...
async def update_appointment(
    ...
    if "notification_methods" in update_data:
        update_data["notification_methods"] = json.dumps(
            [m.value for m in update_data["notification_methods"]]
        )
...
async def update_appointment_status(
    ...
    logger.info(
        f"예약 상태 변경: {appointment_id} - {previous_status} -> {status_update.status}"
    )
    ...
    for time_slot in bulk_data.time_slots:
        for _ in range(bulk_data.workers_per_slot):
            ...
            appointment = HealthExamAppointment(
                ...
                notification_methods=json.dumps(
                    [m.value for m in bulk_data.notification_methods]
                ),
...
async def send_appointment_reminders(
    ...
    if reminder_request.appointment_ids:
        query = query.where(
            HealthExamAppointment.id.in_(reminder_request.appointment_ids)
        )
    else:
    ...
    # 백그라운드에서 알림 발송
    background_tasks.add_task(send_notifications_task, appointments, current_user_id)
...
async def send_notifications_task(
    appointments: List[HealthExamAppointment], user_id: str
):
    """백그라운드 알림 발송 작업"""
