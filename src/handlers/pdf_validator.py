...
class PDFCoordinateValidator:
    ...
    def validate_coordinates(
        self, form_type: str, coordinates: Dict, test_data: Dict
    ) -> Dict:
        """좌표 유효성 검증 및 최적화 제안"""
        validation_result = {
            "form_type": form_type,
        ...
        for field_name, coord in coordinates.items():
            if isinstance(coord, dict):
                ...
                for sub_field, (x, y) in coord.items():
                    ...
                    if is_valid:
                        validation_result["valid_coordinates"].append(
                ...
                if is_valid:
                    ...
                    validation_result["invalid_coordinates"].append(
                        {"field": field_name, "coordinate": (x, y), "issue": suggestion}
                    )

        # 좌표 분포 분석
        validation_result["coverage_analysis"] = self._analyze_coordinate_distribution(
            coordinates
        )

        return validation_result

    def _validate_single_coordinate(
        self, field_name: str, x: float, y: float, max_width: float, max_height: float
    ) -> Tuple[bool, str]:
        """단일 좌표 검증"""
        issues = []
        ...
        if not (
            readable_x_start <= x <= readable_x_end
            and readable_y_start <= y <= readable_y_end
        ):
    ...
    def create_coordinate_debug_pdf(
        self,
        form_type: str,
        coordinates: Dict,
        output_path: str,
        ...
        def draw_coordinate_markers(coord_dict, prefix=""):
            for field_name, coord in coord_dict.items():
                if isinstance(coord, dict):
                    ...
                    # 좌표 유효성에 따라 색상 설정
                    a4_width, a4_height = A4
                    is_valid, _ = self._validate_single_coordinate(
                        full_name, x, y, a4_width, a4_height
                    )
                    ...
                    if test_data and field_name in test_data:
                        ...
                        can.drawString(
                            text_x, text_y - 10, f"= {test_data[field_name]}"
                        )
                        can.setFillColor(black)
    ...
    def suggest_coordinate_improvements(
        self, form_type: str, coordinates: Dict
    ) -> Dict:
        """좌표 개선 제안"""
        improvements = {
            "form_type": form_type,
    ...
    def _find_overlapping_coordinates(
        self, coordinates: Dict, threshold: int = 10
    ) -> List:
        """겹치는 좌표 찾기"""
        all_coords = []
...
def run_coordinate_validation():
    ...
    for form_type, coordinates in PDF_FORM_COORDINATES.items():
        ...
        # 검증 실행
        validation_result = validator.validate_coordinates(
            form_type, coordinates, test_data
        )
        ...
        if validation_result["invalid_coordinates"]:
            ...
            for invalid in validation_result["invalid_coordinates"]:
                print(
                    f"  - {invalid['field']}: {invalid['coordinate']} ({invalid['issue']})"
                )

        # 디버그 PDF 생성
        debug_pdf_path = f"/tmp/debug_{form_type}.pdf"
        validator.create_coordinate_debug_pdf(
            form_type, coordinates, debug_pdf_path, test_data
        )
