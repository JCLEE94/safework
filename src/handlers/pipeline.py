...
async def update_pipeline_status(
    ...
    try:
        ...
        await cache_service.set(cache_key, json.dumps(status_data), expire=86400)  # 24 hours
        
        logger.info(f"Pipeline status updated: {status_update.commit} -> {status_update.status}")
...
async def get_pipeline_status(commit: str):
    ...
    try:
        ...
        if datetime.utcnow().replace(tzinfo=timestamp.tzinfo) - timestamp > timedelta(hours=1):
...
async def update_deployment_status(deployment: DeploymentStatus):
    ...
    try:
        ...
        logger.info(f"Deployment status updated: {deployment.commit} -> {deployment.status}")
...
async def get_pipeline_metrics():
    ...
    try:
        ...
        recent_statuses = [
            ...
            if datetime.fromisoformat(status["timestamp"].replace('Z', '+00:00')) > cutoff_time.replace(tzinfo=None)
        ]
        
        total_pipelines = len(recent_statuses)
        successful_pipelines = len([s for s in recent_statuses if s["status"] == "success"])
