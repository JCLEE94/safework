...
from ..models.settings import SettingsHistory, SystemSettings, UserSettings
from ..schemas.settings import (AllSettingsResponse, AllSettingsUpdate,
                                SettingsHistoryResponse, SystemSettingsCreate,
...
async def get_system_settings(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"시스템 설정 조회 중 오류가 발생했습니다: {str(e)}"
        )
...
async def update_system_settings(
    ...
    try:
        ...
        if not current_settings:
            ...
            if isinstance(settings_data, SystemSettingsUpdate):
                ...
                create_data = SystemSettingsCreate(
                    ...
                    sms_notifications=(
                        settings_data.sms_notifications
                    ...
                    audit_log_enabled=(
                        settings_data.audit_log_enabled
                        if settings_data.audit_log_enabled is not None
                        else True
                    ),
                    password_policy_enabled=(
                        settings_data.password_policy_enabled
                        if settings_data.password_policy_enabled is not None
                        else True
                    ),
                    session_timeout=settings_data.session_timeout or 120,
                    api_access_enabled=(
                        settings_data.api_access_enabled
                        if settings_data.api_access_enabled is not None
                        else False
                    ),
                    maintenance_mode=(
                ...
                new_settings = SystemSettings(
                    **create_data.model_dump(), updated_by=user_id
                )
            else:
                new_settings = SystemSettings(
                    **settings_data.model_dump(), updated_by=user_id
                )
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"시스템 설정 수정 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_user_settings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"사용자 설정 조회 중 오류가 발생했습니다: {str(e)}"
        )
...
async def update_user_settings(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"사용자 설정 수정 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_all_settings(
    ...
    try:
        ...
        return AllSettingsResponse(
            system=(
                SystemSettingsResponse.model_validate(system_settings)
                if system_settings
                else None
            ),
            user=(
                UserSettingsResponse.model_validate(user_settings)
                if user_settings
                else None
            ),
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"설정 조회 중 오류가 발생했습니다: {str(e)}"
        )
...
async def update_all_settings(
    ...
    try:
        ...
        return AllSettingsResponse(system=updated_system, user=updated_user)

    except Exception as e:
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"설정 수정 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_settings_history(
    ...
    try:
        ...
        return [
            SettingsHistoryResponse.model_validate(history) for history in history_list
        ]

    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"설정 변경 이력 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def get_system_status(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"시스템 상태 확인 중 오류가 발생했습니다: {str(e)}"
        )
