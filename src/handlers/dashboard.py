...
from src.models import (AccidentReport, ChemicalSubstance, HealthEducation,
                        HealthExam, WorkEnvironment, Worker)
from src.models.chemical_substance import HazardClass
from src.models.health_education import (EducationStatus,
...
async def get_dashboard_data(db: AsyncSession = Depends(get_db)):
    ...
    on_leave_workers_result = await db.execute(
        select(func.count(Worker.id)).where(Worker.is_active == False)
    )
    on_leave_workers = on_leave_workers_result.scalar()

    health_risk_workers_result = await db.execute(
        select(func.count(Worker.id)).where(Worker.health_status == "caution")
    )
    ...
    completed_this_month_result = await db.execute(
        select(func.count(HealthExam.id)).where(
            HealthExam.exam_date >= thirty_days_ago, HealthExam.exam_date <= now
        )
    )
    ...
    next_exam_result = await db.execute(
        select(HealthExam)
        .where(HealthExam.next_exam_date >= now)
        .order_by(HealthExam.next_exam_date)
    )
    ...
    completed_education_result = await db.execute(
        select(func.count(HealthEducationAttendance.id)).where(
            HealthEducationAttendance.status == EducationStatus.COMPLETED
        )
    )
    completed_education = completed_education_result.scalar()
    completion_rate = (
        (completed_education / total_education * 100) if total_education > 0 else 0
    )

    # 사고 통계
    accidents_this_month_result = await db.execute(
        select(func.count(AccidentReport.id)).where(
            AccidentReport.accident_datetime >= thirty_days_ago
        )
    )
    ...
    hazardous_chemicals_result = await db.execute(
        select(func.count(ChemicalSubstance.id)).where(
            ChemicalSubstance.hazard_class.in_(
                [HazardClass.TOXIC, HazardClass.CARCINOGENIC]
            )
        )
    )
    ...
    return {
        ...
        "health_exams": {
            ...
            "next_scheduled": (
                next_exam.next_exam_date.isoformat() if next_exam else None
            ),
        },
        "work_environment": {
            "last_measurement": (
                last_measurement.measurement_date.isoformat()
                if last_measurement
                else ""
            ),
            "next_due": (
                (last_measurement.measurement_date + timedelta(days=180)).isoformat()
                if last_measurement
                else ""
            ),
            "status": "정상",
