...
# PDF 양식 설정 import
from ..config.pdf_forms import (AVAILABLE_PDF_FORMS, FIELD_LABELS,
                                PDF_FORM_COORDINATES)
# 캐싱 서비스 import
from ..services.cache import CacheTTL, cache_pdf_result, get_cached_pdf
...
DOCUMENT_BASE_DIR = (
    Path(settings.document_dir) if settings.document_dir else current_dir / "document"
)
TEMPLATES_DIR = (
    current_dir / "src" / "templates"
    if (current_dir / "src" / "templates").exists()
    else Path("/app/src/templates")
)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.returncode != 0:
                ...
                return create_form_template_pdf(
                    os.path.splitext(os.path.basename(office_file_path))[0]
                )

            # 변환된 PDF 파일 찾기
            pdf_filename = (
                os.path.splitext(os.path.basename(office_file_path))[0] + ".pdf"
            )
        ...
        if field_name in data and data[field_name]:
            ...
            if value:
                ...
                try:
                    ...
                    print(
                        f"Drew '{value}' at ({adjusted_x}, {adjusted_y}) for {field_name}"
                    )
        ...
        if "original_pdf" in template_info:
            ...
            if original_pdf_path.exists():
                ...
                if page_num < len(reader.pages):
                    ...
                    print(
                        f"PDF generated successfully with original form, size: {len(pdf_content)} bytes"
                    )
                    return pdf_content
...
async def preview_form_base64(form_id: str):
    ...
    try:
        ...
        if source_path and Path(source_path).exists():
            ...
            pdf_stream = create_blank_pdf_with_title(
                form_info.get("source_file", form_id)
            )
...
async def upload_document(file: UploadFile = File(...), category: str = Form(...)):
    ...
    try:
        ...
        if file_extension not in allowed_extensions:
            raise HTTPException(
                status_code=400,
                detail="지원하지 않는 파일 형식입니다. (xlsx, xls, docx, doc, pdf, hwp만 가능)",
            )
...
async def refresh_pdf_forms():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"양식 목록 새로고침 실패: {str(e)}"
        )
...
async def edit_pdf_fields(form_id: str, field_updates: Dict):
    ...
    try:
        ...
        if source_path and Path(source_path).exists():
            ...
            # 빈 PDF 생성
            base_pdf_stream = create_blank_pdf_with_title(
                form_info.get("source_file", form_id)
            )
...
def apply_field_edits(
    pdf_stream: io.BytesIO, field_updates: Dict, form_id: str
) -> io.BytesIO:
    """PDF 필드 편집사항 적용"""
    try:
        # 기존 PDF 읽기
...
def apply_text_edits(
    pdf_stream: io.BytesIO, text_edits: Dict, form_id: str
) -> io.BytesIO:
    """PDF 텍스트 편집사항 적용"""
    try:
        # 기존 PDF 읽기
...
async def get_available_pdf_forms():
    ...
    try:
        ...
        return {"status": "success", "forms": forms, "total": len(forms)}
    except Exception as e:
...
async def get_pdf_template(form_id: str):
    ...
    try:
        ...
        if not template_path.exists():
            raise HTTPException(
                status_code=404, detail="템플릿 파일을 찾을 수 없습니다"
            )
    ...
    try:
        ...
        for field_name in form_data["fields"]:
            field_info = {
                ...
                "type": "text",  # 기본값
                "required": field_name
                in ["company_name", "date", "manager"],  # 필수 필드
            }
...
async def edit_pdf_with_fields(
    form_id: str, template_path: str, field_data: Dict[str, str]
) -> str:
    """PDF 편집 핵심 로직"""
    try:
        # 출력 파일 경로 생성
        ...
        if not coordinates:
            ...
            await create_coordinate_based_pdf(
                template_path, output_path, field_data, coordinates
            )
...
async def create_simple_pdf_overlay(
    template_path: str, output_path: str, field_data: Dict[str, str]
):
...
async def create_coordinate_based_pdf(
    template_path: str, output_path: str, field_data: Dict[str, str], coordinates: Dict
):
    """좌표 기반 정확한 PDF 편집"""
    try:
        packet = io.BytesIO()
...
async def upload_and_edit_pdf(
    file: UploadFile = File(...), field_data: str = Form(...)
):
    """PDF 파일 업로드 후 편집"""
    try:
        # 파일 유효성 검사
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="필드 데이터 형식이 올바르지 않습니다"
            )
...
async def get_documents_stats():
    ...
    try:
        ...
        for category_id, category_name in DOCUMENT_CATEGORIES.items():
            category_path = DOCUMENT_BASE_DIR / category_name
            category_stats = {"count": 0, "size": 0, "name": category_name}
