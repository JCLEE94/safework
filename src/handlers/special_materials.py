...
from ..models.special_materials import (ControlMeasure, ExposureAssessment,
                                        ExposureLevel, MonitoringStatus,
                                        SpecialMaterial,
                                        SpecialMaterialMonitoring,
                                        SpecialMaterialType,
                                        SpecialMaterialUsage)
from ..schemas.special_materials import (ControlMeasureCreate,
                                         ControlMeasureResponse,
                                         DepartmentSpecialMaterialStats,
                                         ExposureAssessmentCreate,
                                         ExposureAssessmentResponse,
                                         PaginatedMonitoringResponse,
                                         PaginatedSpecialMaterialResponse,
                                         PaginatedUsageResponse,
                                         SpecialMaterialCreate,
                                         SpecialMaterialFilter,
                                         SpecialMaterialMonitoringCreate,
                                         SpecialMaterialMonitoringResponse,
                                         SpecialMaterialResponse,
                                         SpecialMaterialStatistics,
                                         SpecialMaterialUpdate,
                                         SpecialMaterialUsageCreate,
                                         SpecialMaterialUsageResponse)
...
async def get_special_materials(
    ...
    try:
        ...
        offset = (page - 1) * size
        query = (
            query.offset(offset).limit(size).order_by(SpecialMaterial.created_at.desc())
        )
        ...
        # 응답 데이터 변환
        items = [
            SpecialMaterialResponse.model_validate(material) for material in materials
        ]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"특별관리물질 목록 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def create_special_material(
    ...
    try:
        ...
        existing_result = await db.execute(
            select(SpecialMaterial).where(
                SpecialMaterial.material_code == material_data.material_code
            )
        )
        ...
        # 새 물질 생성
        new_material = SpecialMaterial(**material_data.model_dump(), created_by=user_id)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"특별관리물질 등록 중 오류가 발생했습니다: {str(e)}",
        )


@router.get("/{material_id}", response_model=SpecialMaterialResponse)
async def get_special_material(material_id: UUID, db: AsyncSession = Depends(get_db)):
    """특별관리물질 상세 조회"""
    try:
        ...
        if not material:
            raise HTTPException(
                status_code=404, detail="특별관리물질을 찾을 수 없습니다"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"특별관리물질 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def update_special_material(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"특별관리물질 수정 중 오류가 발생했습니다: {str(e)}",
        )
...
async def get_usage_records(
    ...
    try:
        ...
        if location:
            conditions.append(
                SpecialMaterialUsage.usage_location.ilike(f"%{location}%")
            )
        if start_date:
        ...
        query = (
            query.offset(offset)
            .limit(size)
            .order_by(SpecialMaterialUsage.usage_date.desc())
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"사용 기록 목록 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def create_usage_record(
    ...
    try:
        ...
        # 새 사용 기록 생성
        new_usage = SpecialMaterialUsage(**usage_data.model_dump(), recorded_by=user_id)
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"사용 기록 등록 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_exposure_assessments(
    ...
    try:
        ...
        for row in rows:
            ...
            assessment_dict = ExposureAssessmentResponse.model_validate(
                assessment
            ).model_dump()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"노출 평가 목록 조회 중 오류가 발생했습니다: {str(e)}",
        )


@router.post("/exposure-assessments", response_model=ExposureAssessmentResponse)
async def create_exposure_assessment(
    assessment_data: ExposureAssessmentCreate, db: AsyncSession = Depends(get_db)
):
    """노출 평가 등록"""
    try:
        # 물질 존재 확인
        material_result = await db.execute(
            select(SpecialMaterial).where(
                SpecialMaterial.id == assessment_data.material_id
            )
        )
        material = material_result.scalar_one_or_none()
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"노출 평가 등록 중 오류가 발생했습니다: {str(e)}"
        )
...
async def get_monitoring_records(
    ...
    try:
        ...
        if monitoring_type:
            conditions.append(
                SpecialMaterialMonitoring.monitoring_type.ilike(f"%{monitoring_type}%")
            )
        if overdue_only:
            conditions.append(
                and_(
                    SpecialMaterialMonitoring.scheduled_date < datetime.now(),
                    SpecialMaterialMonitoring.status.in_(
                        [MonitoringStatus.PENDING, MonitoringStatus.IN_PROGRESS]
                    ),
                )
            )
        ...
        query = (
            query.offset(offset)
            .limit(size)
            .order_by(SpecialMaterialMonitoring.scheduled_date.desc())
        )
        ...
        for row in rows:
            ...
            monitoring_dict = SpecialMaterialMonitoringResponse.model_validate(
                monitoring
            ).model_dump()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"모니터링 기록 목록 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def create_monitoring_record(
    ...
    try:
        ...
        material_result = await db.execute(
            select(SpecialMaterial).where(
                SpecialMaterial.id == monitoring_data.material_id
            )
        )
        material = material_result.scalar_one_or_none()
        ...
        new_monitoring = SpecialMaterialMonitoring(
            **monitoring_data.model_dump(), created_by=user_id
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"모니터링 기록 등록 중 오류가 발생했습니다: {str(e)}",
        )
...
async def get_special_material_statistics(db: AsyncSession = Depends(get_db)):
    """특별관리물질 통계 조회"""
    try:
        # 총 물질 수
        ...
        type_result = await db.execute(
            select(
                SpecialMaterial.material_type, func.count(SpecialMaterial.id)
        ...
        prohibited_result = await db.execute(
            select(func.count(SpecialMaterial.id)).where(
                SpecialMaterial.is_prohibited == True
            )
        )
        ...
        permit_result = await db.execute(
            select(func.count(SpecialMaterial.id)).where(
                SpecialMaterial.requires_permit == True
            )
        )
        ...
        high_risk_result = await db.execute(
            select(func.count(ExposureAssessment.id)).where(
                ExposureAssessment.exposure_level.in_(
                    [ExposureLevel.HIGH, ExposureLevel.CRITICAL]
                )
            )
        )
        ...
        overdue_result = await db.execute(
            select(func.count(SpecialMaterialMonitoring.id)).where(
                and_(
                    SpecialMaterialMonitoring.scheduled_date < datetime.now(),
                    SpecialMaterialMonitoring.status.in_(
                        [MonitoringStatus.PENDING, MonitoringStatus.IN_PROGRESS]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"특별관리물질 통계 조회 중 오류가 발생했습니다: {str(e)}",
        )
    ...
    try:
        ...
        dept_result = await db.execute(
            select(
                ...
                func.count(func.distinct(SpecialMaterialUsage.material_id)).label(
                    "materials_used"
                ),
        ...
        for row in dept_result.fetchall():
            ...
            compliance_rate = (
                (compliant_monitoring / total_monitoring * 100)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"부서별 특별관리물질 통계 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def get_control_measures(
    ...
    try:
        # 기본 쿼리
        query = select(
            ControlMeasure, SpecialMaterial.material_name_korean.label("material_name")
        ).join(SpecialMaterial)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"관리 조치 목록 조회 중 오류가 발생했습니다: {str(e)}",
        )
...
async def create_control_measure(
    ...
    try:
        ...
        material_result = await db.execute(
            select(SpecialMaterial).where(
                SpecialMaterial.id == measure_data.material_id
            )
        )
        material = material_result.scalar_one_or_none()
        ...
        # 새 관리 조치 생성
        new_measure = ControlMeasure(**measure_data.model_dump(), created_by=user_id)
        ...
        await db.rollback()
        raise HTTPException(
            status_code=500, detail=f"관리 조치 등록 중 오류가 발생했습니다: {str(e)}"
        )
