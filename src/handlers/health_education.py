...
from src.schemas.health_education import (AttendanceCreate, AttendanceUpdate,
                                          EducationStatistics,
...
async def create_health_education(
    education_data: HealthEducationCreate, db: AsyncSession = Depends(get_db)
):
    """보건교육 등록"""
...
async def get_education_statistics(db: AsyncSession = Depends(get_db)):
    ...
    # Total hours
    total_hours = (
        await db.scalar(select(func.sum(HealthEducation.education_hours))) or 0
    )

    # Total attendees
    total_attendees = await db.scalar(select(func.count(HealthEducationAttendance.id)))
    ...
    completed_attendances = await db.scalar(
        select(func.count(HealthEducationAttendance.id)).where(
            HealthEducationAttendance.status == "COMPLETED"
        )
    )
    completion_rate = (
        (completed_attendances / total_attendees * 100) if total_attendees > 0 else 0
    )
...
async def delete_health_education(
...
async def add_attendance(
    ...
    for attendance_data in attendance_list:
        ...
        existing = await db.scalar(
            select(HealthEducationAttendance).where(
                and_(
        ...
        attendance = HealthEducationAttendance(
            education_id=education_id, **attendance_data.model_dump()
        )
...
async def get_worker_education_history(
    ...
    required_hours = (
        8 if worker.hire_date.year == current_year else 12
    )  # New: 8hrs, Regular: 12hrs/year
...
async def upload_education_materials(
    education_id: int, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)
):
    """교육 자료 업로드"""
    education = await db.get(HealthEducation, education_id)
    if not education:
    ...
    return {"message": "교육 자료가 업로드되었습니다", "file_path": file_path}
