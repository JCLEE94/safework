...
async def create_health_exam(
    ...
    if exam_data.vital_signs:
        vital_signs = VitalSigns(
            exam_id=exam.id,
            **exam_data.vital_signs.model_dump()
        )
        db.add(vital_signs)
...
async def list_health_exams(
    ...
    query = select(HealthExam).options(
        selectinload(HealthExam.vital_signs),
        selectinload(HealthExam.lab_results)
    )
...
async def get_exams_due_soon(
    days: int = Query(30, description="일수 이내"),
    db: AsyncSession = Depends(get_db)
):
    """건강진단 예정자 목록 조회"""
    ...
    query = (
        ...
        .where(
            or_(
                subquery.c.latest_exam_date + timedelta(days=365) <= datetime.utcnow() + timedelta(days=days),
                subquery.c.latest_exam_date + timedelta(days=180) <= datetime.utcnow() + timedelta(days=days)
            )
        )
    )
...
async def get_exam_statistics(db: AsyncSession = Depends(get_db)):
    ...
    type_stats = await db.execute(
        select(
            ...
            func.count(HealthExam.id).label("count")
        )
        .group_by(HealthExam.exam_type)
    )
    
    # Results distribution
    result_stats = await db.execute(
        select(
            HealthExam.exam_result,
            func.count(HealthExam.id).label("count")
        )
    ...
    yearly_count = await db.scalar(
        select(func.count(HealthExam.id))
        .where(func.extract('year', HealthExam.exam_date) == current_year)
    )
    
    # Workers with followup required
    followup_count = await db.scalar(
        select(func.count(HealthExam.id))
        .where(HealthExam.followup_required == 'Y')
    )
...
async def get_latest_exam_for_worker(worker_id: int, db: AsyncSession = Depends(get_db)):
    """근로자의 최신 건강진단 기록 조회"""
    result = await db.execute(
        select(HealthExam)
        .options(selectinload(HealthExam.vital_signs))
