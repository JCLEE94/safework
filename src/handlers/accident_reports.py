...
from src.schemas.accident_report import (AccidentReportCreate,
...
async def get_accident_statistics(db: AsyncSession = Depends(get_db)):
    ...
    total_days_lost = (
        await db.scalar(select(func.sum(AccidentReport.work_days_lost))) or 0
    )
    ...
    total_cost_result = await db.execute(
        select(
            func.sum(
                AccidentReport.medical_cost
                + AccidentReport.compensation_cost
                + AccidentReport.other_cost
            ).label("total")
        )
    )
    ...
    type_stats = await db.execute(
        select(
            AccidentReport.accident_type, func.count(AccidentReport.id).label("count")
    ...
    investigation_pending = await db.scalar(
        select(func.count(AccidentReport.id)).where(
            AccidentReport.investigation_status.in_(["REPORTED", "INVESTIGATING"])
        )
    )
...
async def get_safety_metrics(
    ...
    days_lost = (
        await db.scalar(
            select(func.sum(AccidentReport.work_days_lost)).where(
                and_(
                    AccidentReport.accident_datetime >= year_start,
                    AccidentReport.accident_datetime <= year_end,
                )
    ...
    total_work_hours = (
        settings.annual_work_days
        * settings.daily_work_hours
        * settings.default_worker_count
    )
    frequency_rate = (
        (year_accidents / total_work_hours) * 1000000 if total_work_hours > 0 else 0
    )
    ...
    prev_year_accidents = await db.scalar(
        select(func.count(AccidentReport.id)).where(
            and_(
                ...
                AccidentReport.accident_datetime
    ...
    improvement_rate = (
        ((prev_year_accidents - year_accidents) / prev_year_accidents * 100)
        if prev_year_accidents > 0
        else 0
    )
        ...
        .where(
            and_(
                or_(
                    ...
                    AccidentReport.work_days_lost
                    >= settings.work_days_lost_threshold,  # 휴업일수 기준
                ),
        ...
        unreported_list.append(
            {
                ...
                "reporting_deadline_passed": days_since * 24
                > settings.accident_report_deadline_hours,  # 신고 기한
            }
        )

    return {"total_unreported": len(unreported_list), "accidents": unreported_list}
...
async def get_accident_report(report_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not report:
        raise HTTPException(
            status_code=404, detail="산업재해 신고서를 찾을 수 없습니다"
        )
