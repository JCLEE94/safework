...
async def view_document(file_path: str):
    ...
    try:
        ...
        if full_path.suffix.lower() != ".pdf":
            raise HTTPException(
                status_code=415, detail="PDF 파일만 뷰어에서 볼 수 있습니다"
            )

        return FileResponse(
            path=str(full_path), media_type="application/pdf", filename=full_path.name
        )
...
async def edit_document_pdf(
    file_path: str, field_data: str = Form(...), field_mappings: str = Form(None)
):
    """기존 PDF 문서 편집"""
    try:
        full_path = document_manager.get_file_path(file_path)
        ...
        # PDF 편집 실행
        edited_pdf = processor.fill_pdf_precisely(
            pdf_content, field_data_dict, field_mappings_dict
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"PDF 편집 중 오류가 발생했습니다: {str(e)}"
        )
...
async def download_document(file_path: str):
    ...
    try:
        ...
        media_type = mime_types.get(
            full_path.suffix.lower(), "application/octet-stream"
        )
                    ...
                    categories.append(
                        {
                            "name": item.name,
                            "path": str(
                                item.relative_to(document_manager.document_dir)
                            ),
    ...
    try:
        ...
        for file_path in file_paths:
            try:
                ...
                results.append(
                    {"file_path": file_path, "status": "error", "message": str(e)}
                )
