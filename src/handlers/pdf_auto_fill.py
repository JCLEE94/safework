...
from ..config.pdf_forms import FIELD_LABELS, PDF_FORM_COORDINATES
...
class PDFAutoFiller:
            ...
            if "AcroForm" in doc.catalog:
                ...
                if "Fields" in acroform:
                    for field_ref in acroform["Fields"]:
                        ...
                        field_name = field.get("T", b"").decode(
                            "utf-8", errors="ignore"
                        )
    ...
    async def auto_fill_pdf(
        ...
        try:
            ...
            # 2. 지능적 필드 매핑
            mapped_data = self.map_fields_intelligently(all_detected_fields, user_data)
...
async def auto_fill_pdf(
    file: UploadFile = File(...), data: str = Form(...)  # JSON 문자열로 받음
):
    """PDF 자동 채우기"""
    if not file.filename.endswith(".pdf"):
        raise HTTPException(status_code=400, detail="PDF 파일만 지원됩니다")
...
async def suggest_field_mapping(
    file: UploadFile = File(...), target_form: Optional[str] = None
):
