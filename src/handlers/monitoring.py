...
async def get_metrics(
    ...
    if not metrics_collector:
        raise HTTPException(
            status_code=503, detail="모니터링 서비스가 초기화되지 않았습니다"
        )
...
async def get_metrics_history(
    ...
    return {"period_minutes": minutes, "count": len(history), "metrics": history}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if not metrics_collector:
        await websocket.send_json({"error": "모니터링 서비스가 초기화되지 않았습니다"})
    ...
    try:
        ...
        while True:
            ...
            if recent_alerts:
                ...
                if new_alerts:
                    await websocket.send_json({"type": "alert", "data": new_alerts})
...
async def get_monitoring_summary(
    ...
    if not recent_metrics:
        return {"message": "충분한 데이터가 수집되지 않았습니다", "data_points": 0}

    # CPU 사용률 통계
    cpu_values = [
        m["system"]["cpu"]["percent"] for m in recent_metrics if "system" in m
    ]

    # 메모리 사용률 통계
    memory_values = [
        m["system"]["memory"]["percent"] for m in recent_metrics if "system" in m
    ]

    # 디스크 사용률 (보통 변화가 적음)
    disk_values = [
        m["system"]["disk"]["percent"] for m in recent_metrics if "system" in m
    ]
...
async def list_docker_containers():
    ...
    try:
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=500, detail=f"Docker 명령어 실행 실패: {result.stderr}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"컨테이너 목록 조회 실패: {str(e)}"
        )


async def stream_logs_generator(
    container_name: str, lines: int, since: Optional[str]
...
async def get_container_logs(
    container_name: str,
    lines: int = Query(
        default=100, ge=1, le=1000, description="가져올 로그 라인 수 (1-1000)"
    ),
    follow: bool = Query(default=False, description="실시간 로그 스트리밍 여부"),
    since: Optional[str] = Query(
        default=None, description="특정 시간 이후 로그 (예: 2h, 1d)"
    ),
):
    ...
    try:
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=404,
                detail=f"컨테이너를 찾을 수 없거나 로그 조회 실패: {result.stderr}",
            )
...
async def get_container_stats(container_name: str):
    ...
    try:
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=404, detail=f"컨테이너를 찾을 수 없음: {result.stderr}"
            )

        lines = result.stdout.strip().split("\n")
        if len(lines) < 2:
            raise HTTPException(
                status_code=404, detail="컨테이너 통계를 가져올 수 없음"
            )
...
async def stream_container_logs(websocket: WebSocket, container_name: str):
    ...
    try:
        ...
        try:
            await websocket.send_json(
                {"type": "error", "message": f"로그 스트리밍 에러: {str(e)}"}
            )
        except:
            pass
    finally:
