...
class DatabaseOptimizer:
    ...
    async def create_indexes(session: AsyncSession):
        ...
        try:
            for index_sql in indexes:
                ...
                logger.info(f"Index created/verified: {index_sql.split('idx_')[1].split(' ')[0]}")
    ...
    async def analyze_tables(session: AsyncSession):
        ...
        """
        tables = [
            "workers", "health_exams", "work_environments",
            "health_education", "chemical_substances", "accident_reports"
        ]
    ...
    @staticmethod
    def optimize_query_for_pagination(query: Select, page: int, page_size: int) -> Select:
        """
        페이지네이션을 위한 쿼리 최적화
    ...
    async def create_materialized_views(session: AsyncSession):
        ...
        try:
            ...
            await session.execute(
                text("CREATE INDEX IF NOT EXISTS idx_worker_health_summary_id ON worker_health_summary(id);")
            )
            await session.execute(
                text("CREATE INDEX IF NOT EXISTS idx_monthly_statistics_month ON monthly_statistics(month);")
            )
