...
from ..models.legal_forms import LegalFormStatus, LegalFormPriority, LegalFormCategory
...
class LegalFormFieldSchema(BaseModel):
    ...
    validation_rules: Optional[Dict[str, Any]] = Field(None, description="유효성 검사 규칙")
...
class LegalFormCreate(BaseModel):
    ...
    form_name_korean: str = Field(..., description="한글 서식명", min_length=1, max_length=200)
    ...
    required_fields: List[LegalFormFieldSchema] = Field(default_factory=list, description="필수 입력 필드")
    submission_deadline: Optional[datetime] = Field(None, description="제출 마감일")
    regulatory_basis: Optional[str] = Field(None, description="법적 근거", max_length=500)
    template_path: Optional[str] = Field(None, description="템플릿 파일 경로", max_length=500)
    priority: LegalFormPriority = Field(default=LegalFormPriority.MEDIUM, description="우선순위")
...
class LegalFormUpdate(BaseModel):
    """법정서식 수정 스키마"""
    form_name: Optional[str] = Field(None, description="서식명", min_length=1, max_length=200)
    form_name_korean: Optional[str] = Field(None, description="한글 서식명", min_length=1, max_length=200)
    category: Optional[LegalFormCategory] = Field(None, description="서식 분류")
    department: Optional[str] = Field(None, description="담당 부서", max_length=100)
    description: Optional[str] = Field(None, description="서식 설명")
    required_fields: Optional[List[LegalFormFieldSchema]] = Field(None, description="필수 입력 필드")
    submission_deadline: Optional[datetime] = Field(None, description="제출 마감일")
    regulatory_basis: Optional[str] = Field(None, description="법적 근거", max_length=500)
    template_path: Optional[str] = Field(None, description="템플릿 파일 경로", max_length=500)
...
class LegalFormResponse(BaseModel):
    ...
    attachments: List[LegalFormAttachmentSchema] = Field(default_factory=list, description="첨부파일")
    approval_history: List[LegalFormApprovalSchema] = Field(default_factory=list, description="승인 이력")
...
class UnifiedDocumentUpdate(BaseModel):
    """통합 문서 수정 스키마"""
    title: Optional[str] = Field(None, description="문서 제목", min_length=1, max_length=255)
