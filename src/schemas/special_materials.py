...
from ..models.special_materials import (ControlMeasureType, ExposureLevel,
                                        MonitoringStatus, SpecialMaterialType)
...
class SpecialMaterialCreate(BaseModel):
    ...
    material_code: str = Field(
        ..., description="물질 코드", min_length=1, max_length=50
    )
    material_name: str = Field(..., description="물질명", min_length=1, max_length=255)
    material_name_korean: str = Field(
        ..., description="한글명", min_length=1, max_length=255
    )
    ...
    hazard_classification: Optional[str] = Field(
        None, description="유해성 분류", max_length=500
    )
    ghs_classification: Optional[Dict[str, Any]] = Field(None, description="GHS 분류")

    # 법적 기준
    occupational_exposure_limit: Optional[Decimal] = Field(
        None, description="작업환경측정 기준"
    )
    short_term_exposure_limit: Optional[Decimal] = Field(
        None, description="단기노출기준"
    )
    ceiling_limit: Optional[Decimal] = Field(None, description="최고노출기준")
    biological_exposure_index: Optional[Decimal] = Field(
        None, description="생물학적 노출지수"
    )
    ...
    physical_state: Optional[str] = Field(
        None, description="물리적 상태", max_length=50
    )
...
class SpecialMaterialUpdate(BaseModel):
    ...
    material_name: Optional[str] = Field(
        None, description="물질명", min_length=1, max_length=255
    )
    material_name_korean: Optional[str] = Field(
        None, description="한글명", min_length=1, max_length=255
    )
    cas_number: Optional[str] = Field(None, description="CAS 번호", max_length=50)
    ...
    monitoring_frequency_days: Optional[int] = Field(
        None, description="측정 주기(일)", ge=1
    )
    health_exam_frequency_months: Optional[int] = Field(
        None, description="건강진단 주기(월)", ge=1
    )
...
class SpecialMaterialResponse(BaseModel):
    ...
    total_monitoring_records: Optional[int] = Field(
        None, description="총 모니터링 기록 수"
    )
    last_monitoring_date: Optional[datetime] = Field(
        None, description="마지막 모니터링 일시"
    )
...
class SpecialMaterialUsageCreate(BaseModel):
    ...
    usage_location: str = Field(
        ..., description="사용 장소", min_length=1, max_length=255
    )
    usage_purpose: str = Field(
        ..., description="사용 목적", min_length=1, max_length=500
    )
    ...
    exposure_duration_hours: Optional[Decimal] = Field(
        None, description="노출 시간(시간)", ge=0
    )
    ...
    ventilation_type: Optional[str] = Field(
        None, description="환기 방식", max_length=100
    )
...
class ExposureAssessmentCreate(BaseModel):
    ...
    assessment_type: str = Field(
        ..., description="평가 유형", min_length=1, max_length=100
    )
    assessment_location: str = Field(
        ..., description="평가 장소", min_length=1, max_length=255
    )
    work_activity: str = Field(
        ..., description="작업 활동", min_length=1, max_length=500
    )

    # 측정 결과
    measured_concentration: Optional[Decimal] = Field(
        None, description="측정 농도", ge=0
    )
    measurement_unit: Optional[str] = Field(
        None, description="측정 단위", max_length=20
    )
    sampling_duration_minutes: Optional[int] = Field(
        None, description="시료채취 시간(분)", ge=1
    )
    sampling_method: Optional[str] = Field(
        None, description="시료채취 방법", max_length=200
    )
    analysis_method: Optional[str] = Field(
        None, description="분석 방법", max_length=200
    )

    # 노출 평가
    exposure_level: ExposureLevel = Field(..., description="노출 수준")
    exposure_route: Optional[str] = Field(
        None, description="주요 노출 경로", max_length=100
    )
    risk_score: Optional[Decimal] = Field(None, description="위험도 점수", ge=0, le=10)

    # 관리 조치 권고
    recommended_controls: Optional[List[str]] = Field(
        None, description="권고 관리 조치"
    )
    ...
    assessor_name: str = Field(
        ..., description="평가자명", min_length=1, max_length=100
    )
    assessor_qualification: Optional[str] = Field(
        None, description="평가자 자격", max_length=200
    )
    assessment_organization: Optional[str] = Field(
        None, description="평가기관", max_length=200
    )
...
class SpecialMaterialMonitoringCreate(BaseModel):
    ...
    monitoring_type: str = Field(
        ..., description="모니터링 유형", min_length=1, max_length=100
    )
    location: str = Field(
        ..., description="모니터링 장소", min_length=1, max_length=255
    )

    # 모니터링 계획
    scheduled_date: datetime = Field(..., description="예정일")
    frequency_type: str = Field(
        ..., description="주기 유형", min_length=1, max_length=50
    )
    next_monitoring_date: Optional[datetime] = Field(
        None, description="다음 모니터링 예정일"
    )
    ...
    exceedance_factor: Optional[Decimal] = Field(
        None, description="기준 초과배수", ge=0
    )
    ...
    action_responsible: Optional[str] = Field(
        None, description="시정조치 담당자", max_length=100
    )
    action_status: Optional[str] = Field(
        None, description="시정조치 상태", max_length=50
    )

    # 담당자 정보
    monitor_name: str = Field(
        ..., description="모니터링 담당자", min_length=1, max_length=100
    )
    monitor_organization: Optional[str] = Field(
        None, description="모니터링 기관", max_length=200
    )
    report_number: Optional[str] = Field(
        None, description="보고서 번호", max_length=100
    )
...
class PaginatedMonitoringResponse(BaseModel):
    ...
    items: List[SpecialMaterialMonitoringResponse] = Field(
        ..., description="모니터링 목록"
    )
    total: int = Field(..., description="전체 개수")
    page: int = Field(..., description="현재 페이지")
