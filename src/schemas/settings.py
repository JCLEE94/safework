...
from ..models.settings import (BackupFrequency, DashboardLayout, Language,
                               ReportLanguage, Theme)
...
class SystemSettingsBase(BaseModel):
    ...
    company_phone: Optional[str] = Field(
        None, description="회사 전화번호", max_length=50
    )
    company_registration_number: Optional[str] = Field(
        None, description="사업자등록번호", max_length=50
    )
    ...
    safety_manager: Optional[str] = Field(
        None, description="안전관리자", max_length=100
    )
    health_manager: Optional[str] = Field(
        None, description="보건관리자", max_length=100
    )
    representative_name: Optional[str] = Field(
        None, description="대표자명", max_length=100
    )
    ...
    data_retention_period: int = Field(
        36, description="데이터 보존 기간(개월)", ge=1, le=120
    )
...
class SystemSettingsUpdate(BaseModel):
    ...
    company_name: Optional[str] = Field(
        None, description="회사명", min_length=1, max_length=255
    )
    company_address: Optional[str] = Field(None, description="회사 주소")
    company_phone: Optional[str] = Field(
        None, description="회사 전화번호", max_length=50
...
class UserSettingsBase(BaseModel):
    ...
    notification_preferences: NotificationPreferences = Field(
        default_factory=NotificationPreferences, description="알림 설정"
    )
    ...
    def validate_dashboard_layout(cls, v):
        ...
        if v not in ["default", "compact", "detailed"]:
            raise ValueError(
                "대시보드 레이아웃은 default, compact, detailed 중 하나여야 합니다"
            )
        return v
...
class UserSettingsUpdate(BaseModel):
    ...
    user_name: Optional[str] = Field(
        None, description="사용자명", min_length=1, max_length=100
    )
    user_email: Optional[str] = Field(None, description="이메일", max_length=255)
    user_phone: Optional[str] = Field(None, description="전화번호", max_length=50)
    user_role: Optional[str] = Field(None, description="역할", max_length=100)
