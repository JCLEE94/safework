...
from ..models.checklist import (ChecklistPriority, ChecklistStatus,
                                ChecklistType)
...
class ChecklistTemplateCreate(BaseModel):
    ...
    items: List[ChecklistTemplateItemSchema] = Field(
        default_factory=list, description="체크리스트 항목"
    )
...
class ChecklistTemplateUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, description="템플릿명", min_length=1, max_length=255
    )
    name_korean: Optional[str] = Field(
    ...
    frequency_days: Optional[int] = Field(None, description="주기(일)", ge=1)
    legal_basis: Optional[str] = Field(None, description="법적 근거", max_length=500)
    items: Optional[List[ChecklistTemplateItemSchema]] = Field(
        None, description="체크리스트 항목"
    )
...
class ChecklistTemplateResponse(BaseModel):
    ...
    items: List[ChecklistTemplateItemResponse] = Field(
        default_factory=list, description="체크리스트 항목"
    )
...
class ChecklistInstanceCreate(BaseModel):
    ...
    priority: ChecklistPriority = Field(
        ChecklistPriority.MEDIUM, description="우선순위"
    )
...
class ChecklistInstanceUpdate(BaseModel):
    ...
    assignee: Optional[str] = Field(
        None, description="담당자", min_length=1, max_length=100
    )
    department: Optional[str] = Field(None, description="담당 부서", max_length=100)
    ...
    notes: Optional[str] = Field(None, description="비고")
    location: Optional[str] = Field(None, description="점검 장소", max_length=255)
    items: Optional[List[ChecklistInstanceItemCheck]] = Field(
        None, description="점검 항목들"
    )
...
class ChecklistInstanceResponse(BaseModel):
    ...
    updated_at: datetime = Field(..., description="수정일시")
    created_by: str = Field(..., description="생성자")
    items: List[ChecklistInstanceItemResponse] = Field(
        default_factory=list, description="점검 항목들"
    )
...
class ChecklistScheduleCreate(BaseModel):
    ...
    default_assignee: Optional[str] = Field(
        None, description="기본 담당자", max_length=100
    )
    default_department: Optional[str] = Field(
        None, description="기본 담당 부서", max_length=100
    )
    ...
    def validate_frequency_type(cls, v):
        ...
        if v not in ["daily", "weekly", "monthly", "yearly"]:
            raise ValueError(
                "반복 유형은 daily, weekly, monthly, yearly 중 하나여야 합니다"
            )
        return v
