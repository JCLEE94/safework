...
class NotificationService:
    ...
    async def send_push(
        self, device_token: str, title: str, message: str, data: Optional[Dict] = None
    ) -> Dict:
        """앱 푸시 알림 발송"""
    ...
    async def send_bulk_notifications(
        ...
        for recipient in recipients:
            ...
            if "email" in methods and recipient.get("email"):
                try:
                    subject = (
                        template_data.get("subject", "건설업 보건관리 시스템 알림")
                        if template_data
                        else "알림"
                    )
                    ...
                    logger.error(
                        f"이메일 발송 실패: {recipient.get('email')} - {str(e)}"
                    )
            ...
            if "kakao" in methods and recipient.get("phone"):
                try:
                    ...
                    result = await self.send_kakao(
                        recipient["phone"], message, template_id
                    )
                    recipient_results["kakao"] = result
                except Exception as e:
                    logger.error(
                        f"카카오톡 발송 실패: {recipient.get('phone')} - {str(e)}"
                    )
            ...
            if "app_push" in methods and recipient.get("device_token"):
                try:
                    title = (
                        template_data.get("push_title", "건강진단 알림")
                        if template_data
                        else "알림"
                    )
                    ...
                    logger.error(
                        f"푸시 알림 발송 실패: {recipient.get('device_token')} - {str(e)}"
                    )
                    recipient_results["app_push"] = {
                        "status": "failed",
                        "error": str(e),
                    }
    ...
    async def send_template_notification(
        self, template_type: str, recipient: Dict, template_data: Dict
    ) -> Dict:
        """템플릿 기반 알림 발송"""
        # 템플릿 유형별 메시지 생성
        templates = {
        ...
        for method in methods:
            if method == "sms" and recipient.get("phone"):
                ...
                results["email"] = await self.send_email(
                    recipient["email"], template["subject"], message
                )
            elif method == "kakao" and recipient.get("phone"):
                results["kakao"] = await self.send_kakao(
                    recipient["phone"], message, template_data.get("kakao_template_id")
                )
