...
class WorkerService:
    ...
    async def create_worker(
        self, db: AsyncSession, worker_data: WorkerCreate
    ) -> Worker:
        """근로자 생성 (비즈니스 로직 포함)"""
        try:
            # 1. 사번 중복 검사
    ...
    async def update_worker(
        self, db: AsyncSession, *, worker_id: int, worker_data: WorkerUpdate
    ) -> Optional[Worker]:
        """근로자 정보 업데이트"""
        try:
            # 1. 기존 근로자 조회
            ...
            # 3. 비즈니스 규칙 적용
            translated_data = self._apply_business_rules(
                translated_data, is_update=True
            )
            ...
            logger.info(
                f"근로자 정보 업데이트 완료: {updated_worker.name}({updated_worker.employee_id})"
            )
    ...
    async def get_workers_by_health_status(
        self, db: AsyncSession, *, health_status: str, skip: int = 0, limit: int = 100
    ) -> Tuple[List[Worker], int]:
        """건강상태별 근로자 조회"""
        try:
            # 한국어 상태를 영어로 번역
    ...
    async def update_health_status(
        ...
        *,
        ...
        try:
            ...
            korean_status = self.translator.get_korean_label(
                "health_status", english_status
            )
    ...
    async def get_workers_needing_health_exam(
        self, db: AsyncSession, *, months_since_last_exam: int = 12
    ) -> List[Worker]:
        """건강진단이 필요한 근로자 조회"""
        try:
    ...
    async def get_comprehensive_statistics(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            enhanced_stats = {
                ...
                "by_gender": self._add_korean_labels(stats["by_gender"], "gender"),
    ...
    def _add_korean_labels(
        self, stats: Dict[str, int], field_type: str
        ...
        for key, count in stats.items():
            ...
            labeled_stats[key] = {"count": count, "label": korean_label, "english": key}
