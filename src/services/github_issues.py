...
"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 201:
                        ...
                        print(
                            f"GitHub 이슈 생성 완료: #{result['number']} - {result['html_url']}"
                        )
                        return result
                    else:
                        error_text = await response.text()
                        print(
                            f"GitHub 이슈 생성 실패: {response.status} - {error_text}"
                        )
                        return None
    ...
    async def report_frontend_error(self, error_data: Dict[str, Any]) -> Optional[Dict]:
        """프론트엔드 에러 리포트"""
