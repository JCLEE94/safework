...
class IntegrationTestRedisCache:
    ...
    async def test_cache_stampede_prevention(self):
        ...
        async with create_test_environment() as env:
            ...
            results = await asyncio.gather(*[get_or_compute(i) for i in range(10)])
            duration = (datetime.now() - start_time).total_seconds()

            # 검증: 한 번만 계산되어야 함
            unique_timestamps = set(
                r.get("timestamp", "") for r in results if "timestamp" in r
            )
    ...
    async def test_cache_memory_management(self):
        ...
        async with create_test_environment() as env:
            ...
            assert (
                existing_count >= 95
            ), f"저장된 키가 너무 적습니다: {existing_count}/100"
    ...
    async def test_cache_performance_metrics(self):
        ...
        async with create_test_environment() as env:
            ...
            metrics = {"hits": 0, "misses": 0, "set_times": [], "get_times": []}
            ...
            assert (
                avg_set_time < 10
            ), f"SET 평균 시간이 너무 깁니다: {avg_set_time:.2f}ms"
            assert (
                avg_get_time < 5
            ), f"GET 평균 시간이 너무 깁니다: {avg_get_time:.2f}ms"
