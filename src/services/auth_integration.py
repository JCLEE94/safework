...
from src.testing import (
    ...
    assert_response_ok, assert_error_response
)
...
class IntegrationTestAuthentication:
        ...
        async with create_test_environment() as env:
            ...
            roles = {
                "admin": {
                    ...
                    "allowed": ["/api/v1/workers/", "/api/v1/settings/", "/api/v1/reports/"],
                    "forbidden": []
                },
                "manager": {
    ...
    async def test_department_based_permissions(self):
        ...
        async with create_test_environment() as env:
            ...
            # 부서별 사용자 생성
            dept_users = [
                {"id": 10, "email": "dev@test.com", "role": "manager", "department": "개발부"},
                {"id": 11, "email": "hr@test.com", "role": "manager", "department": "인사부"},
                {"id": 12, "email": "safety@test.com", "role": "manager", "department": "안전관리팀"}
            ]
            ...
            async with db.get_session() as session:
                ...
                workers = [
                    ...
                    Worker(name="안전관리1", employee_id="SAFE001", department="안전관리팀")
                ]
            ...
            for user in dept_users:
                ...
                if "items" in workers_list:
                    for worker in workers_list["items"]:
                        assert worker["department"] == user["department"], \
    ...
    async def test_token_validation_performance(self):
        ...
        async with create_test_environment() as env:
            ...
            assert avg_duration < 50, f"평균 응답 시간이 너무 깁니다: {avg_duration:.2f}ms"
    ...
    async def test_brute_force_protection(self):
        ...
        async with create_test_environment() as env:
            ...
            assert failed_attempts >= max_attempts, "무차별 대입 공격 방어가 작동하지 않습니다"
