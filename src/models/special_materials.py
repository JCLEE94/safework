...
class SpecialMaterial(Base):
    ...
    material_type = Column(
        SQLEnum(SpecialMaterialType), nullable=False, comment="물질 유형"
    )
    ...
    # 법적 기준
    occupational_exposure_limit = Column(
        Numeric(10, 6), nullable=True, comment="작업환경측정 기준(ppm/mg/m³)"
    )
    short_term_exposure_limit = Column(
        Numeric(10, 6), nullable=True, comment="단기노출기준"
    )
    ceiling_limit = Column(Numeric(10, 6), nullable=True, comment="최고노출기준")
    biological_exposure_index = Column(
        Numeric(10, 6), nullable=True, comment="생물학적 노출지수"
    )

    # 관리 정보
    is_prohibited = Column(
        Boolean, nullable=False, default=False, comment="사용금지 여부"
    )
    requires_permit = Column(
        Boolean, nullable=False, default=True, comment="허가 필요 여부"
    )
    monitoring_frequency_days = Column(
        Integer, nullable=False, default=180, comment="측정 주기(일)"
    )
    health_exam_frequency_months = Column(
        Integer, nullable=False, default=12, comment="건강진단 주기(월)"
    )
    ...
    monitoring_records = relationship(
        "SpecialMaterialMonitoring", back_populates="material"
    )
...
class SpecialMaterialUsage(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    material_id = Column(
        UUID(as_uuid=True), ForeignKey("special_materials.id"), nullable=False
    )
    ...
    exposure_duration_hours = Column(
        Numeric(5, 2), nullable=True, comment="노출 시간(시간)"
    )
...
class ExposureAssessment(Base):
    ...
    sampling_duration_minutes = Column(
        Integer, nullable=True, comment="시료채취 시간(분)"
    )
    ...
    priority_level = Column(
        String(20), nullable=False, default="medium", comment="우선순위"
    )
    follow_up_required = Column(
        Boolean, nullable=False, default=False, comment="후속조치 필요"
    )
...
class SpecialMaterialMonitoring(Base):
    ...
    next_monitoring_date = Column(
        DateTime, nullable=True, comment="다음 모니터링 예정일"
    )
...
class ControlMeasure(Base):
    ...
    # 조치 정보
    measure_type = Column(
        SQLEnum(ControlMeasureType), nullable=False, comment="조치 유형"
    )
