...
class ChecklistInstance(Base):
    ...
    # 상태 및 우선순위
    status = Column(SQLEnum(ChecklistStatus), nullable=False, default=ChecklistStatus.PENDING, comment="상태")
    priority = Column(SQLEnum(ChecklistPriority), nullable=False, default=ChecklistPriority.MEDIUM, comment="우선순위")
    ...
    items = relationship("ChecklistInstanceItem", back_populates="instance", cascade="all, delete-orphan")
    attachments = relationship("ChecklistAttachment", back_populates="instance", cascade="all, delete-orphan")
...
class ChecklistInstanceItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    instance_id = Column(UUID(as_uuid=True), ForeignKey("checklist_instances.id"), nullable=False)
    template_item_id = Column(UUID(as_uuid=True), ForeignKey("checklist_template_items.id"), nullable=False)
...
class ChecklistSchedule(Base):
    ...
    # 반복 설정
    frequency_type = Column(String(20), nullable=False, comment="반복 유형 (daily/weekly/monthly/yearly)")
    ...
    # 자동 생성 설정
    auto_create_days_before = Column(Integer, nullable=False, default=7, comment="자동 생성 일수 (사전)")
    reminder_days_before = Column(Integer, nullable=False, default=3, comment="알림 일수 (사전)")
