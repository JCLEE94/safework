...
class SystemSettings(Base):
    ...
    company_registration_number = Column(
        String(50), nullable=True, comment="사업자등록번호"
    )
    ...
    # 백업 설정
    auto_backup_enabled = Column(
        Boolean, nullable=False, default=True, comment="자동 백업 활성화"
    )
    backup_frequency = Column(
        String(20), nullable=False, default="daily", comment="백업 주기"
    )

    # 알림 설정
    notification_enabled = Column(
        Boolean, nullable=False, default=True, comment="알림 활성화"
    )
    email_notifications = Column(
        Boolean, nullable=False, default=True, comment="이메일 알림"
    )
    sms_notifications = Column(
        Boolean, nullable=False, default=False, comment="SMS 알림"
    )

    # 보고서 설정
    report_language = Column(
        String(20), nullable=False, default="korean", comment="보고서 언어"
    )

    # 데이터 관리
    data_retention_period = Column(
        Integer, nullable=False, default=36, comment="데이터 보존 기간(개월)"
    )
    audit_log_enabled = Column(
        Boolean, nullable=False, default=True, comment="감사 로그 활성화"
    )

    # 보안 설정
    password_policy_enabled = Column(
        Boolean, nullable=False, default=True, comment="비밀번호 정책 활성화"
    )
    session_timeout = Column(
        Integer, nullable=False, default=120, comment="세션 타임아웃(분)"
    )
    api_access_enabled = Column(
        Boolean, nullable=False, default=False, comment="API 접근 허용"
    )

    # 시스템 관리
    maintenance_mode = Column(
        Boolean, nullable=False, default=False, comment="유지보수 모드"
    )
...
class UserSettings(Base):
    ...
    notification_preferences = Column(
        JSON, nullable=False, default=dict, comment="알림 설정"
    )

    # UI 설정
    dashboard_layout = Column(
        String(20), nullable=False, default="default", comment="대시보드 레이아웃"
    )
