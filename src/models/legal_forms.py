...
class LegalForm(Base):
    ...
    form_code = Column(
        String(50), unique=True, nullable=False, index=True, comment="서식 코드"
    )
    ...
    required_fields = Column(
        JSON, nullable=False, default=list, comment="필수 입력 필드"
    )
    ...
    # 상태 및 우선순위
    status = Column(
        SQLEnum(LegalFormStatus),
        nullable=False,
        default=LegalFormStatus.DRAFT,
        comment="서식 상태",
    )
    priority = Column(
        SQLEnum(LegalFormPriority),
        nullable=False,
        default=LegalFormPriority.MEDIUM,
        comment="우선순위",
    )
    ...
    attachments = relationship(
        "LegalFormAttachment", back_populates="legal_form", cascade="all, delete-orphan"
    )
    approval_history = relationship(
        "LegalFormApproval", back_populates="legal_form", cascade="all, delete-orphan"
    )
...
class LegalFormField(Base):
    ...
    legal_form_id = Column(
        UUID(as_uuid=True), ForeignKey("legal_forms.id"), nullable=False
    )
...
class UnifiedDocument(Base):
    ...
    access_level = Column(
        String(50), nullable=False, default="public", comment="접근 권한"
    )
