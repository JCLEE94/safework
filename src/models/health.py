...
class HealthExam(Base):
    ...
    vital_signs = relationship(
        "VitalSigns", back_populates="health_exam", cascade="all, delete-orphan"
    )
    lab_results = relationship(
        "LabResult", back_populates="health_exam", cascade="all, delete-orphan"
    )
...
class VitalSigns(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    health_exam_id = Column(
        Integer,
        ForeignKey("health_exams.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
