...
class ChemicalSubstance(Base):
    ...
    # Management
    special_management_material = Column(
        String(1), default="N", index=True
    )  # 특별관리물질 여부
    ...
    __table_args__ = (
        ...
        Index(
            "ix_chemicals_hazard_special", "hazard_class", "special_management_material"
        ),
        Index("ix_chemicals_location_quantity", "storage_location", "current_quantity"),
        Index(
            "ix_chemicals_safety_flags", "carcinogen", "mutagen", "reproductive_toxin"
        ),
    )
...
class ChemicalUsageRecord(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    chemical_id = Column(
        Integer, ForeignKey("chemical_substances.id", ondelete="CASCADE")
    )
