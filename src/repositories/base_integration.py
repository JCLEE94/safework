...
from src.models import ChemicalSubstance, HealthExam, WorkEnvironment, Worker
from src.testing import (create_test_environment, integration_test,
    ...
    async def test_transaction_rollback(self):
        ...
        async with create_test_environment() as env:
            ...
            async with db.get_session() as session:
                # 트랜잭션 시작
                worker = Worker(
                    name="롤백테스트", employee_id="ROLLBACK_001", department="테스트부"
                )
                session.add(worker)
    ...
    async def test_concurrent_updates(self):
        ...
        async with create_test_environment() as env:
            ...
            await asyncio.gather(
                update_department("영업부"), update_health_status("요관찰")
            )
    ...
    async def test_complex_queries(self):
        ...
        async with create_test_environment() as env:
            ...
            async with db.get_session() as session:
                ...
                # 각 부서별 정규직 수
                result = await session.execute(
                    select(Worker.department, func.count(Worker.id).label("count"))
                ...
                subquery = (
                    ...
                    .where(Worker.health_status == "요관찰")
                    .distinct()
                    .subquery()
                )

                result = await session.execute(
                    select(Worker).where(Worker.department.in_(select(subquery)))
                )
                ...
                assert all(
                    w.employment_type == "정규직"
                    and w.health_status == "정상"
                    and w.department in ["건설부", "안전관리팀"]
    ...
    async def test_date_timezone_handling(self):
        ...
        async with create_test_environment() as env:
            ...
            async with db.get_session() as session:
                ...
                result = await session.execute(
                    select(Worker).where(
                        Worker.birth_date.between(date(2024, 1, 1), date(2024, 12, 31))
                    )
                )
