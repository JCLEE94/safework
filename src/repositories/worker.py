    ...
    async def get_workers_by_department(
        ...
        try:
            ...
            count_query = select(func.count(Worker.id)).where(
                Worker.department.ilike(f"%{department}%")
            )
    ...
    async def get_statistics(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            gender_stats = await db.execute(
                select(Worker.gender, func.count(Worker.id)).group_by(Worker.gender)
            )
            gender_counts = {
                row[0] or "미분류": row[1] for row in gender_stats.fetchall()
            }

            # 고용형태별 통계
            employment_stats = await db.execute(
                select(Worker.employment_type, func.count(Worker.id)).group_by(
            ...
            health_stats = await db.execute(
                select(Worker.health_status, func.count(Worker.id)).group_by(
            ...
            work_stats = await db.execute(
                select(Worker.work_type, func.count(Worker.id)).group_by(
                    Worker.work_type
                )
            )
    ...
    async def update_health_status(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                f"근로자 {worker.name}({worker.employee_id}) 건강상태 업데이트: {new_status}"
            )
            return worker
