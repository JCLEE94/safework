                        ...
                        if request.headers.get("content-type", "").startswith(
                            ...
                            if self._contains_xss(data):
                                ...
                                return JSONResponse(
                                    status_code=400,
                                    content={
                                        "detail": "Potential XSS detected in request"
                                    },
                                )
                    except Exception:
                        pass
        ...
        if request.query_params:
            for key, value in request.query_params.items():
                if self._contains_sql_injection(str(value)):
                    logger.warning(
                        f"SQL injection attempt in query params from {request.client.host}"
                    )
                    return JSONResponse(
                        status_code=400,
                ...
                try:
                    if request.headers.get("content-type", "").startswith(
                        ...
                        if self._check_json_for_sql(data):
                            logger.warning(
                                f"SQL injection attempt in body from {request.client.host}"
                            )
                            return JSONResponse(
                                status_code=400,
                                content={
...
class APIKeyAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        self.rate_limits[api_key] = [
            timestamp
            for timestamp in self.rate_limits[api_key]
        ...
        if len(self.rate_limits[api_key]) >= self.rate_limit_max:
            return JSONResponse(
                status_code=429, content={"detail": "API rate limit exceeded"}
            )
...
class IPWhitelistMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, whitelist: Optional[Set[str]] = None, enabled: bool = False
    ):
        super().__init__(app)
        import os
        ...
        self.enabled = (
            enabled or os.getenv("IP_WHITELIST_ENABLED", "false").lower() == "true"
        )
        ...
        if client_ip not in self.whitelist:
            ...
            if forwarded_for:
                ...
                if real_ip not in self.whitelist:
                    logger.warning(
                        f"Blocked request from non-whitelisted IP: {real_ip}"
                    )
                    return JSONResponse(
