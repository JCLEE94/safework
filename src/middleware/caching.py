...
from ..services.cache import cache_service, CacheTTL
        ...
        if response.status_code == 200:
            try:
                ...
                try:
                    ...
                    logger.debug(f"API 응답 캐시 저장: {request.url.path}, TTL: {ttl}초")
...
class CacheInvalidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (self._should_invalidate(request) and 
            ...
            try:
                ...
                for pattern in patterns:
                    deleted_count = await cache_service.delete_pattern(f"health_cache:{pattern}*")
                    if deleted_count > 0:
                        logger.info(f"캐시 무효화: {pattern} 패턴, 삭제된 키: {deleted_count}개")
