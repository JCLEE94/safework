...
class JWTAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if settings.environment == "development" or settings.disable_auth:
            logger.info(
                f"Skipping authentication for {request.url.path} - development mode"
            )
            return await call_next(request)
                ...
                for session_key in sessions:
                    ...
                    if data:
                        ...
                        session_times.append(
                            (session_key, session_data.get("last_activity", 0))
                        )
    ...
    async def _update_session_activity(self, user_id: str, request: Request):
        ...
        try:
            ...
            await cache_service.redis_client.setex(
                session_key, self.session_timeout, json.dumps(session_data)
            )
