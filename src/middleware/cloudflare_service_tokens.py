...
class CloudflareServiceTokenMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if cf_access_client_id and cf_access_client_secret:
            ...
            try:
                ...
                token_info = await self._validate_service_token(
                    cf_access_client_id, cf_access_client_secret, request.url.path
                )
                ...
                try:
                    from ..services.service_token_metrics import \
                        ServiceTokenMetrics

                    ServiceTokenMetrics.record_request(
                        token_info["name"], request.url.path, "success"
                    )
                    ServiceTokenMetrics.record_validation_duration(
                        token_info["name"], validation_duration
                    )
                except ImportError:
                ...
                try:
                    ...
                    ServiceTokenMetrics.record_request(
                        "unknown", request.url.path, "failure"
                    )
                except ImportError:
                    pass
        ...
        if hasattr(request.state, "service_token"):
            response.headers["X-Service-Token-Name"] = request.state.service_token[
                "name"
            ]
            response.headers["X-Service-Token-Validation-Duration"] = str(
                request.state.validation_duration
            )

        return response

    async def _validate_service_token(
        self, client_id: str, client_secret: str, endpoint: str
    ) -> Dict[str, Any]:
    ...
    async def _validate_with_cloudflare(
        ...
        if not self.cloudflare_team_domain:
            logger.warning(
                "Cloudflare team domain not configured, skipping API validation"
            )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Cloudflare validation failed: {response.status_code}"
                    )
                    return False
...
class ServiceTokenRateLimitMiddleware(BaseHTTPMiddleware):
            ...
            if token_name in self.rate_limits:
                ...
                if not is_within_limit:
                    ...
                    try:
                        ...
                        ServiceTokenMetrics.record_request(
                            token_name, request.url.path, "rate_limited"
                        )
                    except ImportError:
                        pass
    ...
    async def _check_rate_limit(
        self, client_id: str, requests: int, window: int
    ) -> bool:
...
class ServiceTokenAuditMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if hasattr(request.state, "service_token"):
            ...
            logger.info("Service token audit log", extra=audit_data)
...
async def validate_service_token_permissions(
    client_id: str, required_scopes: List[str]
) -> bool:
