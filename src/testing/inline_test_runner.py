...
async def run_inline_tests(
    module_name: Optional[str] = None, verbose: bool = True
) -> List[TestResult]:
    """
    ...
    for module in modules_to_test:
        ...
        for test_func in tests:
            ...
            result = TestResult(
                name=test_name, module=module, passed=False, duration=0.0
            )

            try:
                # Create test instance if needed
                if (
                    inspect.ismethod(test_func)
                    or "self" in inspect.signature(test_func).parameters
                ):
                    # Get the class
                    test_class = None
