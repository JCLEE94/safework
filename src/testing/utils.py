...
def assert_response_ok(
    response: Response, expected_status: int = 200, check_json: bool = True
) -> Dict[str, Any]:
    """
...
def assert_cache_invalidated(cache_key: str, cache_service: Any, timeout: float = 1.0):
    """
...
def assert_datetime_format(datetime_str: str, field_name: str = "datetime"):
    ...
    try:
        ...
        raise AssertionError(f"{field_name} must be in ISO format, got: {datetime_str}")
...
def generate_test_file(
    filename: str, content: str = "Test content", size_kb: Optional[int] = None
) -> bytes:
