...
from src.config.pdf_forms import (
...
class TestPdfEditorAPI:
    ...
    def test_edit_pdf_form(self, mock_edit_function, client):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
             patch('src.handlers.documents.FileResponse') as mock_file_response:
            
            response = client.post(
                f"/api/v1/pdf-editor/forms/{form_id}/edit",
                data=form_data
            )
    ...
    def test_upload_and_edit_pdf(self, client, mock_pdf_file):
        ...
        with patch('src.handlers.documents.edit_uploaded_pdf') as mock_edit, \
             patch('src.handlers.documents.FileResponse') as mock_file_response:
            
            mock_edit.return_value = "/tmp/test_output.pdf"
            
            response = client.post(
                "/api/v1/pdf-editor/upload-and-edit",
                files=files,
                data=data
            )
...
class TestPdfEditing:
    ...
    def test_simple_pdf_overlay(self, mock_writer, mock_reader, mock_canvas):
        ...
        with patch('builtins.open', mock_open()):
            # 비동기 함수를 동기적으로 테스트
            import asyncio
            asyncio.run(create_simple_pdf_overlay(template_path, output_path, field_data))
    ...
    def test_korean_font_setup(self, mock_font_setup):
        ...
        # 모의 캔버스 객체
        mock_canvas = type('MockCanvas', (), {
            'setFont': lambda self, name, size: None
        })()
...
class TestPdfEditorIntegration:
    ...
    def test_complete_pdf_editing_workflow(self, client):
        ...
        # 4. PDF 편집 요청 (모의 환경)
        with patch('src.handlers.documents.edit_pdf_with_fields') as mock_edit, \
             patch('src.handlers.documents.FileResponse') as mock_response, \
