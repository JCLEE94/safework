...
from src.handlers.documents import (
    fill_pdf_form, create_text_overlay, FORM_TEMPLATES,
    PDF_FORM_COORDINATES, get_available_pdf_forms
)
...
class TestPDFFormFunctionality:
    ...
    def test_pdf_form_coordinates_completeness(self):
        ...
        for form_id in required_forms:
            assert form_id in PDF_FORM_COORDINATES, f"Missing form coordinates: {form_id}"
            ...
            for field_name, field_info in form_config["fields"].items():
                assert "x" in field_info, f"Missing x coordinate for {form_id}.{field_name}"
                assert "y" in field_info, f"Missing y coordinate for {form_id}.{field_name}"
                assert "label" in field_info, f"Missing label for {form_id}.{field_name}"
    ...
    def test_coordinate_validation(self):
        ...
        for form_id, form_config in PDF_FORM_COORDINATES.items():
            for field_name, field_info in form_config["fields"].items():
                ...
                assert 0 <= x <= A4_WIDTH, f"Invalid x coordinate for {form_id}.{field_name}: {x}"
                assert 0 <= y <= A4_HEIGHT, f"Invalid y coordinate for {form_id}.{field_name}: {y}"
                
                # Check reasonable positioning (not too close to edges)
                assert x >= 20, f"X coordinate too close to left edge: {form_id}.{field_name}"
                assert y >= 20, f"Y coordinate too close to bottom edge: {form_id}.{field_name}"
                assert x <= A4_WIDTH - 20, f"X coordinate too close to right edge: {form_id}.{field_name}"
                assert y <= A4_HEIGHT - 20, f"Y coordinate too close to top edge: {form_id}.{field_name}"
