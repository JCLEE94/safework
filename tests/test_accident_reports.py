...
async def test_update_accident_report(client: AsyncClient, test_accident_report):
    ...
    response = await client.put(f"/api/v1/accident-reports/{test_accident_report.id}", json=update_data)
    assert response.status_code == 200
...
async def test_filter_accidents_by_severity(client: AsyncClient, test_accident_report):
    """사고 심각도별 필터링 테스트"""
    response = await client.get(f"/api/v1/accident-reports/?severity={test_accident_report.severity.value}")
    assert response.status_code == 200
