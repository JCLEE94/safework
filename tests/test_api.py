...
def test_workers_api():
    ...
    if response.status_code == 200:
        ...
        print(
            f"Get Workers List: {response.status_code}, Count: {len(response.json()['items'])}"
        )
        ...
        if worker_id:
            ...
            response = requests.patch(
                f"{BASE_URL}/workers/{worker_id}", json=update_data
            )
...
def test_health_exams_api():
    ...
    if worker_response.status_code == 200:
        ...
        print(
            f"Get Health Exams: {response.status_code}, Count: {len(response.json())}"
        )
