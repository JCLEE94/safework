...
from src.handlers.documents import (create_form_template_pdf,
                                    create_text_overlay)
        ...
        if pdf_content:
            ...
            try:
                ...
                if reader.pages:
                    page = reader.pages[0]
                    print(
                        f"   첫 페이지 크기: {page.mediabox.width} x {page.mediabox.height}"
                    )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    print(
        "이제 텍스트 오버레이가 정확한 위치에 표시되고 샘플 데이터가 제거된 깔끔한 양식이 생성됩니다."
    )
