...
async def test_get_chemical_substance(client: AsyncClient, test_chemical_substance):
    ...
    response = await client.get(
        f"/api/v1/chemical-substances/{test_chemical_substance.id}"
    )
    assert response.status_code == 200
...
async def test_update_chemical_substance(client: AsyncClient, test_chemical_substance):
    ...
    response = await client.put(
        f"/api/v1/chemical-substances/{test_chemical_substance.id}", json=update_data
    )
    assert response.status_code == 200
...
async def test_record_chemical_usage(
    client: AsyncClient, test_chemical_substance, test_worker
):
    """화학물질 사용 기록 테스트"""
    usage_data = {
    ...
    response = await client.post(
        f"/api/v1/chemical-substances/{test_chemical_substance.id}/usage",
        json=usage_data,
    )
    assert response.status_code == 200
...
async def test_search_chemicals(client: AsyncClient, test_chemical_substance):
    """화학물질 검색 테스트"""
    response = await client.get(
        f"/api/v1/chemical-substances/?search={test_chemical_substance.korean_name}"
    )
    assert response.status_code == 200
    ...
    assert data["total"] >= 1
    assert any(
        item["korean_name"] == test_chemical_substance.korean_name
