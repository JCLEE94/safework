...
class TestPerformance:
    ...
    async def test_search_performance(self, client: AsyncClient):
        """검색 성능 테스트"""
        search_params = {"search": "테스트", "page_size": 20}
...
class TestCachePerformance:
    ...
    async def test_cache_headers(self, client: AsyncClient):
        ...
        cache_headers = ["X-Cache-Status", "Cache-Control", "ETag", "Last-Modified"]
...
class TestDatabasePerformance:
    ...
    async def test_query_performance_headers(self, client: AsyncClient):
        ...
        performance_headers = ["X-Query-Count", "X-Query-Time", "X-Pool-Active"]
        ...
        if initial_response.status_code == 200:
            ...
            if final_response.status_code == 200:
                ...
                memory_increase_percent = (
                    (memory_increase / initial_used) * 100 if initial_used > 0 else 0
                )

                # Allow up to 10% memory increase
                assert (
                    memory_increase_percent < 10
                ), f"Memory increased by {memory_increase_percent}%"
